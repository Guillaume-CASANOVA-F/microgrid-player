...
class Player:
    ...
    def compute_battery_load(self):
        ...
        for t in range(self.horizon):
            ...
            stock = self.stock_initial + self.delta_t * pulp.lpSum([(self.rho_c * variables[s]["battery_load_plus"] - (
        ...
        stock_final = self.stock_initial + self.delta_t * pulp.lpSum([(self.rho_c * variables[s]["battery_load_plus"] - (
                    variables[s]["battery_load_moins"] * (1 / self.rho_d))) for s in
                                               range(self.horizon)])
